...
// helpers
void allToLowerCase(char array[]);
...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // 1st Step = Store the method, request-target and HTTP version in 3
            // separate strings.
            ...
            // Making sure we actually have something in the 3 parts of the request-line
            ...
            // Creating a variable to store the value of the request_target
            // because strtok will be called and we don't want to change the original
            // value.
    
            char* request_target = strdup(req[1]);
                  
            // Make sure the request-target is correct
            
            // Store the absolute_path in a separate variable and the query, if there is one.
            
            char* absolute_path;
            char* query = "";
            
            if (strchr(request_target, '?') != NULL) // If there is a query
            {
                char* ab_path;
            
                if (request_target[strlen(request_target) - 1] != '?') // If there really is query
                {
                    ab_path = strtok(request_target, "?");
                    query = strtok(NULL, "\0");
                }
                
                ab_path = strtok(request_target, "?");
            
                absolute_path = strdup(ab_path);
            } 
            else
            {
            ...
            else if (req[1][0] != '/' || strchr(absolute_path, '.') == NULL) 
            {
                // if request-target does not begin with /, respond to the browser 
                // with 501 Not Implemented;
                
                // if absolute-path does not contain a 
                // . (and thus a file extension), respond to the browser 
                // with 501 Not Implemented
                
                error(501);
                continue;
            } else if (strchr(req[1], '"') != NULL)
            {
                // if request-target contains a ", respond to the browser with 
                // 400 Bad Request;
                error(400);
                continue;
            } else if (strcmp(req[2], "HTTP/1.1") != 0)
            {
                // if HTTP-version is not HTTP/1.1, respond to the browser 
            ...
            // 1st Step
            // Path has to assume the value of root
            
            char path[strlen(root) + strlen(absolute_path)];
            
            strcpy(path, root);
            
            // Concat the path, which now holds the value of root, and absolute_path
            ...
            extension = extension + sizeof(char); // Exclude the dot in the extension.
...
const char* lookup(const char* extension)
{
    //1st Step = Make the extension all lower case
    char lower_extension[strlen(extension)];
    
    strcpy(lower_extension, extension);
    
    allToLowerCase(lower_extension);
    
    // 2nd Step = Make the comparisons.
    
    if (strcmp(lower_extension, "css") == 0)
    {
        return "text/css";
    } 
    else if (strcmp(lower_extension, "html") == 0)
    {
        return "text/html";
    } 
    else if (strcmp(lower_extension, "gif") == 0)
    {
        return "image/gif";
    } 
    else if (strcmp(lower_extension, "ico") == 0)
    {
        return "image/x-icon";
    } 
    else if (strcmp(lower_extension, "jpg") == 0)
    {
        return "image/jpeg";
    } 
    else if (strcmp(lower_extension, "js") == 0)
    {
